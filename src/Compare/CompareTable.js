...
const getMetricValue = (type, card) => {
  switch (type) {
    case 'yearlyFee':
...
const CompareTable = ({ cards, onRemove, onClear }) => {
  ...
  return (
    <section className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm animate-fade-up">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          <h3 className="text-xl font-bold text-slate-900">Vergleich ({cards.length}/3)</h3>
          <p className="text-sm text-slate-600">Beste Werte sind grün markiert.</p>
        </div>
        {onClear && (
          <Button variant="outline" className="rounded-full border-slate-300" onClick={onClear}>
            Vergleich leeren
          </Button>
        )}
      </div>
      ...
      <Table className="overflow-hidden rounded-xl border border-slate-200">
        <TableBody>
          <TableRow>
            <TableCell className="font-semibold text-slate-700">Karte</TableCell>
            {cards.map((card) => (
              <TableCell key={card.Issuer} className="font-semibold text-slate-900">
                <div className="space-y-2">
                  <div className="mx-auto h-14 w-24 rounded-lg border border-slate-200 bg-white p-1">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {onRemove && (
                      <button
                        type="button"
                        className="rounded-full px-2 py-0.5 text-xs text-slate-500 hover:bg-slate-100 hover:text-slate-700"
              ...
              {cards.map((card) => {
                ...
                return (
                  <TableCell
                    ...
                    className={isBest ? 'bg-emerald-50 text-emerald-900 font-semibold' : 'text-slate-700'}
                  >
